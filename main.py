...
# ======================================================================
# 🌟 01.TECH AGGREGATOR - منطقة اختبار معزولة تماماً 🌟
# ======================================================================
...
# 1. مسارات المعاملات المالية (الأساسية لكي لا يرفضنا المزود)
...
async def finish_round_01tech(request: Request, x_request_sign: Optional[str] = Header(None)):
    return {"balance": "0.00"} # سيتم ربطها لاحقاً بشكل كامل، المهم الآن جلب الألعاب
...
# 2. مسار جلب الألعاب (مفلترة إلى كازينو أو لايف)
            ...
            if "providers" in data:
                for provider in data["providers"]:
                    ...
                    for game in provider.get("games", []):
                        # فلترة الألعاب حسب طلبك (لايف أو كازينو عادي)
                        is_live = game.get("live", False)
                        game_cat = game.get("category", "").lower()
                        
                        if category == "live" and not is_live and "live" not in game_cat:
                            continue # تخطي إذا لم تكن لايف
                        if category == "slots" and (is_live or "live" in game_cat):
                            continue # تخطي إذا كانت لايف وأنت طلبت سلوتس
